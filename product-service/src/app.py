...
def get_products():
    ...
    try:
        ...
        log.exception('An exception occur when retrieving all products')
...
def get_product(id):
    ...
    try:
        ...
        log.exception(f'An exception occur when retrieving product{id}')
        return f'An exception occurred while retrieving product{id}', 500


# curl --header "Content/Type: application/json" --request POST --data '{"name": "Product 3"}' -v
...
def put_product(id):
    ...
    try:
        ...
        log.exception(f'An exception occurred while updating product{existing_product.name}')
        return f'An exception occurred while updating product{existing_product.name}', 500
